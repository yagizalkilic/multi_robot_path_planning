...
PhysicalPath::PhysicalPath( int x_boundary, int y_boundary, int AGV_radius, double segment_distance, 
...
PhysicalPath::PhysicalPath( double segment_distance, std::vector<Point> prepared_path )
{   
    this->unprocessed_path = prepared_path;
    this->segment_distance = segment_distance;
    process_path();
 ...
 * line amount and line length has a specified interval. Each segment
 * within the line is the same length.
...
void PhysicalPath::process_path()
{
    ...
    path_subdivision_amount = total_distance / segment_distance;
