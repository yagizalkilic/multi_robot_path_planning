...
Point generate_random_point(std::vector<int> space_side_length, int num_dimensions) 
{
    ...
    {
        point.coordinates.push_back((int)(get_random_double(0, space_side_length[i] - 1)));
    }
    return point;
}
...
/**
 * Calculates the maximum velocity an AGV can have before traversing the given distance if it
 * needs to stop at the end of the path.
 *
 * @param max_acc maximum accleration
 * @param distance
 * @return maximum possible velocity
 */
double calculate_max_speed_before_deacc(double max_acc, double distance)
{
    return std::sqrt(2 * max_acc * distance);
}

/**
 * Calculates the traversed distance during acceleration of an AGV
 *
 * @param start_vel starting velocity
 * @param end_vel ending velocity
 * @param acc acceleration value
 * @param dt acceleration update rate
...
std::pair<double, double> calculate_distance_during_acc(double start_vel, double end_vel, double acc, double dt)
{
    ...
    double cover_time = 0.0;
    double cur_vel = start_vel;
    if ( cur_vel < end_vel )
    {
        while ( cur_vel < end_vel )
        {
            cur_vel += acc * dt;
            cover_time += dt;
            covered_distance += cur_vel * dt;
        }
    }
    else
    {
    ...
    return std::make_pair(covered_distance, cover_time);
}
