    ...
    std::vector<double> ang_vels, const std::string& turtle_name, const turtlesim::Pose& pose, ros::Publisher velocity_publisher)
{
    ...
    for ( size_t i = 0; i < velocities.size(); i++)
    {
        ...
        velMsg.linear.x = velocities[i] / 10.0;
        velMsg.angular.z = 0; 
        ...
        ROS_INFO("Velocity: %f", velocities[i]);
...
/**
 * Callback to process velocities_durations_orientations msg
 * 
 * @param msg
 * @param robot_index robot to associate the infomration with
 */
...
/**
 * Checks if the given schedule is a valid schedule.
 * 
 * @param all_velocities all velocity values in the msg
 * @param all_durations all duration values in the msg
 * @param all_orientations all orientation values in the msg
 * @param all_ang_vels all angular velocity values in the msg
 * @return true if valid schedule, false otherwise
 */
...
/**
 * Checks if two poses are the same with a small tolerance interval.
 * 
 * @param pose1 first pose
 * @param pose2 second pose
 * @param tolerance difference that two poses can have
...
{
    ros::init(argc, argv, "turtle_restrained_subscriber");
    ros::NodeHandle nh;
