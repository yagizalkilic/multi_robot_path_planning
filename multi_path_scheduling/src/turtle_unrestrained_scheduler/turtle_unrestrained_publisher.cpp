...
int main(int argc, char** argv)
{
    ...
    auto velocity_scheduler = UnrestrainedVelocityScheduler(all_paths, path_RRT_star, AGV_max_velocity);
    auto velocity_schedule = velocity_scheduler.get_velocity_duration_orientation();
    ...
    ros::init(argc, argv, "turtle_unrestrained_publisher");
    ros::NodeHandle nh;
    ...
    while (ros::ok())
    {
        ...
        for( int i = 0; i < AGV_amount; i++ )
        {
            ...
            for ( int k = 0; k < velocity_schedule[i].size(); k++ )
            {
                velocities.push_back(velocity_schedule[i][k][0]);
                durations.push_back(velocity_schedule[i][k][1]);
                orientations.push_back(velocity_schedule[i][k][2]);
                angular_velocities.push_back(velocity_schedule[i][k][3]);
