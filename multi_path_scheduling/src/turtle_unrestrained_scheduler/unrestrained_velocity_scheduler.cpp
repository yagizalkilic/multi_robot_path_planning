#include "../../include/unrestrained_velocity_scheduler.h"

UnrestrainedVelocityScheduler::UnrestrainedVelocityScheduler(std::vector<PhysicalPath> physical_paths, 
...
/**
 * Calculates intervals for each individual schedule. Interval size can differ
 * between schedules but interval amount must stay same. Each interval represents
 * one node travelsal.
 */
void UnrestrainedVelocityScheduler::calculate_intervals_and_schedules()
{
    ...
    for ( auto i : temp_schedules)
    {
    	for ( auto n : i )
    	{
    		std::cout << n << " s " << std::endl;
    	}
    	std::cout << "aadfasf" << std::endl;
    }
    for (auto i : physical_paths )
    {
    	std::cout << i.get_final_physical_path_points().size() << " as " << std::endl;
    }
}

/**
 * Calculates the distance of travel between each node or within each interval.
 */
void UnrestrainedVelocityScheduler::calculate_distances_between_intervals()
{
...
/**
 * Using the distance information, calculates the duration of travel between each node taking
 * the max duration as final duration. Then calculates average velocity for each AGV.
 */
void UnrestrainedVelocityScheduler::calculate_average_velocities_and_total_times_per_interval()
{
...
/**
 * Calculates the final schedule information that contains duration, linear velocity,
 * orientation, angular velocity.
 */
void UnrestrainedVelocityScheduler::calculate_final_schedule()
{
	for ( int i = 0; i < physical_path_amount; i++)
	{
...
/**
 * Calculates angular velocities at each point using orientations and durations information.
 */
void UnrestrainedVelocityScheduler::calculate_angular_velocities()
{
...
/**
 * Formats the final schedule information into a publishable state.
 */
std::vector<std::vector<std::vector<double>>> UnrestrainedVelocityScheduler::get_velocity_duration_orientation()
{
